...
import { Plus, RefreshCw, Coffee, MapPin, Layers } from "lucide-react"
...
import { TableMergeDialog } from "./components/table-merge-dialog"

const TableManagement: React.FC = () => {
  const [searchQuery, setSearchQuery] = useState("")
  const [activeFilter, setActiveFilter] = useState<TableStatus | "all" | "Merged">("all")
  ...
  const filteredTables = tables.filter((table) => {
    ...
    // Handle the "Merged" filter type separately
    let matchesFilter = true
    if (activeFilter === "Merged") {
      matchesFilter = !!table.tableMergeId
    } else {
      matchesFilter = activeFilter === "all" || table.status === activeFilter
    }
  ...
  const tableStatusCounts = {
    ...
    Merged: tables.filter((t) => !!t.tableMergeId).length,
  }
  ...
  return (
    <Card className="border-amber-100 shadow-sm">
      ...
      <CardContent className="space-y-3 sm:space-y-4 p-2 sm:p-4 md:p-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-4">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            <Button
              ...
              size="sm"
              className="h-8 sm:h-9 flex-1 sm:flex-none text-xs sm:text-sm bg-amber-600 hover:bg-amber-700 text-white"
            >
            ...
            <Button
              ...
              size="sm"
              className="h-8 sm:h-9 flex-1 sm:flex-none text-xs sm:text-sm bg-purple-600 hover:bg-purple-700 text-white"
            >
        ...
        <TableList tables={filteredTables} activeFilter={activeFilter} onTableUpdated={handleTableUpdated} />
        <TableMergeDialog
