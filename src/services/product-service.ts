...
class ProductService {
  ...
  public async getProductById(id: string): Promise<ApiResponse<ProductModel>> {
    try {
      return await get<ProductModel>(
        `products/${id}?includeProperties=Category%2CProductOptions.Option.OptionItems%2CProductComboSlots%2CRecipes.Ingredient%2CProductComboSlots.ProductComboSlotItems.Product%2CChildProducts`
      )
    } catch (error) {
  ...
  public async getProductsByCategory(categoryId: string): Promise<ApiResponse<ProductsResult>> {
    try {
      return await get<ProductsResult>(`/products?categoryId=${categoryId}&TakeCount=1000`)
    } catch (error) {
